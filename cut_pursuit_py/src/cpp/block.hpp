...
{
public:
	/* Constructor. Arguments are the block size */
	Block(int size) { first = last = nullptr; block_size = size; }
	...
	Type *New(int num = 1)
	{
		...
		if (!last || last->current + num > last->last)
		{
			...
			else
			{
				...
				if (!next) {
                    std::cerr << "Block: not enough memory." << std::endl;
                    exit(EXIT_FAILURE);
                }
	...
	Type *ScanNext(iterator& i)
	{
		while (i.scan_current_data >= i.scan_current_block -> current)
		{
			...
			if (!i.scan_current_block) return nullptr;
			i.scan_current_data = & ( i.scan_current_block -> data[0] );
		}
...
public:
	...
	DBlock(int size){ first = nullptr; first_free = nullptr; block_size = size; }
